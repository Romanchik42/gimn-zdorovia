...
Актуально на 24 сентября 2026 (GIMN-030).
...
│   │   │                     dosage, duration
│   │   ├── progression/      уровень и вес: level-check, weight, offer
│   │   ├── notifications/    каналы уведомлений (работает telegram)
│   │   ├── points/           баллы за вклад тестеров
...
├── supabase/
│   ├── migrations/           0001…0028, применяются по порядку
...
### Появилось в GIMN-029 и GIMN-030

| Путь | Что там |
|---|---|
| `src/lib/auth/otp.ts`, `otp-session.ts`, `otp-handlers.ts` | Вход по одноразовому коду: выдача, проверка, сессия |
| `src/lib/auth/sms.ts`, `email-code.ts`, `oauth.ts` | Каналы доставки кода и готовность провайдеров |
| `src/app/api/auth/{sms,email}/{request,verify}/` | Ручки входа по коду |
| `src/components/auth/code-login.tsx`, `other-logins.tsx` | Ввод номера и кода, свёртка «Другие способы входа» |
| `src/lib/legal/pdf.ts` | Печатная версия документов: вкладывает шрифт, режет на страницы |
| `src/legal/*.ts` | Тексты документов данными; разметка — в `document-view.tsx` |
| `src/legal/registry.ts` | Перечень документов: экран и PDF берут список отсюда |
| `assets/fonts/` | PT Sans для PDF. В `public/` класть нельзя — см. README рядом |
| `src/lib/tour/steps.ts`, `state.ts`, `hints.ts` | Шаги тура, его состояние и контекстные подсказки |
| `src/components/tour/context-hint.tsx` | Подсказка, которая приходит при первом открытии экрана |
| `supabase/migrations/0027`, `0028` | Одноразовые коды, отметки показанных подсказок |

### Появилось в GIMN-028

| Путь | Что там |
|---|---|
| `src/lib/workout-engine/dosage.ts` | Подходы, повторы и вес по уровню и режиму; накладывается поверх подбора |
| `src/lib/workout-engine/duration.ts` | Честный расчёт времени: силовое считается подходами, а не повторами |
| `src/lib/progression/level-check.ts` | Три условия перехода на следующий уровень. Чистые функции, без базы |
| `src/lib/progression/weight.ts` | Шаг веса по факту выполненного и разгрузочная неделя |
| `src/lib/progression/offer.ts` | Предложение уровня после тренировки: отметка, модалка, Telegram |
| `src/lib/notifications/channels.ts` | Один вход для уведомлений. Доставляет telegram, остальные — заглушки |
| `src/app/api/workout/sets/` | Запись факта: подход, повторы, вес, RPE |
| `src/app/api/progression/level-up/` | Ответ на предложение уровня. Условия перепроверяются на сервере |
| `src/components/progression/level-up-dialog.tsx` | Модалка с двумя равноправными кнопками |
| `src/components/progress/body-measurements.tsx` | Последний замер и разница с прошлым по каждому обхвату |
| `supabase/migrations/0021…0025` | Снаряжение зала, «свой вариант», дозировка и подходы, замеры тела, предложение уровня |
| `scripts/check-dosage.mts`, `check-progression.mts` | Проверки арифметики времени и прогрессии |
...
| `DOC_REVMATOLOG_GYM_MVP.md` | Упражнения зала на согласование. Только в ветке `feature/gym-exercises-mvp` (собирается скриптом) |
...
| `node scripts/build-doctor-gym-doc.mjs` | Пересобирает документ по упражнениям зала (в ветке `feature/gym-exercises-mvp`) |
...
| `npm run check:equipment` | Режимы и снаряд: снаряд не протекает в режим Бехтерева (45 проверок). Бывший `check:turnik` |
| `npm run check:points` | Баллы: нет двойного начисления за одну и ту же ссылку (13 проверок) |
| `npm run check:dosage` | Время занятия: силовое считается подходами, а не повторами (21 проверка) |
| `npm run check:progression` | Уровень и вес: пороги зафиксированы числами, уровень идёт только вверх (45 проверок) |
| `npm run check:gym` | Только в ветке `feature/gym-exercises-mvp`: день зала, два снаряда, уровни (50 проверок) |
| `npm run check:auth` | Вход по коду: одноразовость, перебор, частота запросов (41 проверка) |
| `npm run check:pdf` | Печатные документы: кириллица читается из файла, текст не теряется (51 проверка) |
| `npm run check:tour` | Тур: кнопки на каждом шаге, якоря существуют, тексты сверены с батчем (54 проверки) |
...
Основная работа идёт коммитами прямо в `main`. Для проекта из одного
разработчика это не беда; ветка заводится там, где выкладывать нельзя:
...
* `feature/gym-exercises-mvp` — девятнадцать упражнений зала, миграция
  `0026` и генератор дня зала. **В `main` не вливать до согласования с
  ревматологом.** Что ему показать — `DOC_REVMATOLOG_GYM_MVP.md`,
  собирается `node scripts/build-doctor-gym-doc.mjs`.
...
Миграции 0017–0025 применены (последняя — 0025, предложение уровня).
Непримененной остаётся **0026** — она живёт в ветке
`feature/gym-exercises-mvp` и до согласования с ревматологом не
применяется. Код к ней устроен так, что без неё день зала просто не
собирается: наличие упражнений в справочнике — отдельное условие.
