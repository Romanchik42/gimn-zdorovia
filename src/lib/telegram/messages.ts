...
/**
 * Первое сообщение бота (GIMN-026). Длинное — ровно один раз, при первом
 * знакомстве: человек только что пришёл и не знает, что внутри. Дальше бот
 * снова короткий, «домашнее» сообщение в две строки (HOME_TEXT).
 *
 * Дисклеймер здесь обязателен: это одно из трёх мест, где он должен быть
 * (SPEC 5.9) — приложение медицинское, и первое касание не исключение.
 */
export function welcomeText(name: string | undefined, invited = false): string {
  const hello = name?.trim() ? `Здравствуйте, ${name.trim()}!` : "Здравствуйте!";
  return [
    hello,
    "",
    invited
      ? "Вас пригласили в «Гимн.здоровья» — гимнастику для здоровья при болезни Бехтерева и для поддержания формы."
      : "Это «Гимн.здоровья» — гимнастика для здоровья при болезни Бехтерева и для поддержания формы.",
    "",
    "Что внутри:",
    "🧘 Упражнения с понятной техникой и картинкой движения",
    "📅 Программа на каждый день, её можно менять под себя",
    "🥗 Меню на день и список покупок",
    "📈 Прогресс: вес, подвижность, регулярность",
    "⏰ Напоминания в то время, которое выберете",
    "",
    "Программа подстраивается под самочувствие: если на занятии стало тяжело — скажете об этом кнопкой, и следующее станет легче. Обычно это 30 минут в день.",
    "",
    "⚠️ «Гимн.здоровья» не заменяет врача. Перед началом занятий проконсультируйтесь с ревматологом, особенно если болезнь в обострении.",
    "",
    "Нажмите кнопку ниже — вход произойдёт сам.",
  ].join("\n");
}

/**
 * Кнопки первого сообщения: приложение и обратная связь. Вторая ведёт в
 * настройки приложения — форма отзыва оттуда уходит админу в Telegram
 * (GIMN-011), то есть это и есть работающая поддержка.
 */
export function welcomeButtons(ref?: string): InlineButton[][] {
  return [
    [{ text: "Открыть приложение", web_app: { url: webAppUrl("/app", ref) }, style: "primary" }],
    [{ text: "Написать нам", web_app: { url: webAppUrl("/app/settings", ref) } }],
  ];
}

export const HELP_TEXT = [
  "Гимн.здоровья — гимнастика для здоровья. Вот что умеет бот.",
  "",
  "Команды:",
...
export const HOME_TEXT = {
  ...
  /** Короткое «с возвращением» — для повторного /start у зарегистрированных. */
